...
def _is_auto_disabled(feed_id: str) -> bool:
    """检查 RSS 源是否因连续失败被自动禁用"""
    record = _load_health_record()
    info = record.get(feed_id, {})
    consecutive = info.get("consecutive_failures", 0)
    return consecutive >= AUTO_DISABLE_THRESHOLD and info.get("disabled", False)


def _reset_health(feed_id: str) -> None:
    """重置指定 RSS 源的健康状态（用于手动恢复）"""
    record = _load_health_record()
    if feed_id in record:
        del record[feed_id]
...
class RSSFetcher:
    ...
    def _update_health(self, feed_id: str, feed_name: str, success: bool, error: str = "") -> None:
        ...
        if success:
            was_disabled = info.pop("disabled", False)
            info["consecutive_failures"] = 0
            info["last_success"] = now
            if was_disabled:
                print(f"[RSS] {feed_name}: 已自动恢复，重新启用")
        else:
            info["consecutive_failures"] = info.get("consecutive_failures", 0) + 1
            info["total_failures"] = info.get("total_failures", 0) + 1
            info["last_error"] = error
            if info["consecutive_failures"] >= AUTO_DISABLE_THRESHOLD:
                info["disabled"] = True
                print(f"[RSS] {feed_name}: 连续失败 {info['consecutive_failures']} 次，已自动禁用")
    ...
    def fetch_all(self) -> RSSData:
        ...
        for i, feed in enumerate(self.feeds):
            # 检查是否已被自动禁用
            if _is_auto_disabled(feed.id):
                print(f"[RSS] {feed.name}: 已被自动禁用，跳过（删除 output/rss_health.json 可手动恢复）")
                id_to_name[feed.id] = feed.name
                failed_ids.append(feed.id)
                skipped_disabled += 1
                continue

            if i > skipped_disabled:
