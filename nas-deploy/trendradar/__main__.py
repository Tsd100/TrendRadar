...
def _run_preview(config: Dict, platform: str = "feishu") -> None:
    """预览通知内容（不实际发送）

    Args:
        config: 应用配置字典
        platform: 目标平台类型 (feishu, dingtalk, wework, telegram, ntfy, bark, slack)
    """
    ctx = AppContext(config)
    ...
    try:
        ...
        if not all_results:
            print("没有找到当天的数据，请先运行一次完整抓取")
            return

        total_titles = sum(len(titles) for titles in all_results.values())
        print(f"数据: {len(all_results)} 个平台, {total_titles} 条标题")
        ...
        print(f"统计: {len(stats)} 个关键词组, {sum(s['count'] for s in stats)} 条匹配")

        # 3. 准备报告数据
        report_data = ctx.prepare_report(stats, mode="current")
        print(f"报告: {sum(len(s['titles']) for s in report_data['stats'])} 条标题")

        # 4. 生成通知内容
        batches = ctx.split_content(
            report_data, platform, mode="current", report_type="当前榜单"
        )

        # 5. 输出预览
        print(f"\n{'='*60}")
        print(f"{platform.upper()} 通知预览 ({len(batches)} 批次)")
        print(f"{'='*60}")

        for i, batch in enumerate(batches, 1):
            if len(batches) > 1:
                print(f"\n--- 批次 {i}/{len(batches)} ({len(batch)} 字符) ---")
            if len(batch) > 4000:
                print(batch[:4000])
                print(f"... (截断，共 {len(batch)} 字符)")
            else:
                print(batch)
    finally:
        ctx.cleanup()
...
预览命令:
  --preview [平台]       预览通知内容（不实际发送）
                         平台: feishu(默认), dingtalk, wework, telegram, ntfy, bark, slack
...
示例:
  ...
  python -m trendradar --preview          # 预览飞书通知内容
  python -m trendradar --preview dingtalk # 预览钉钉通知内容
"""
    )
    parser.add_argument(
        "--show-schedule",
    ...
    parser.add_argument(
        ...
        const="feishu",
        default=None,
        choices=["feishu", "dingtalk", "wework", "telegram", "ntfy", "bark", "slack"],
        help="预览通知内容（不实际发送），默认 feishu"
    )
    ...
    try:
        ...
        # 处理预览命令
        if args.preview:
            _run_preview(config, args.preview)
            return
