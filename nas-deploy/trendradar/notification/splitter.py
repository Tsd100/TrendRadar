...
def split_content_into_batches(
    ...
    changes_content: Optional[str] = None,
    cross_platform_content: Optional[str] = None,
) -> List[str]:
    ...
    TL;DR、变化对比、跨平台热点内容插入到头部之后、区域之前。
    ...
    Args:
        ...
        tldr_content: TL;DR 摘要卡片内容（已格式化，可选）
        changes_content: 变化对比卡片内容（已格式化，可选）
        cross_platform_content: 跨平台热点卡片内容（已格式化，可选）
    ...
    # 自动生成介绍卡片（如果未预提供）
    if tldr_content is None and report_data.get("stats"):
        try:
            from trendradar.report.summary import generate_tldr_for_report
            tldr_content = generate_tldr_for_report(report_data, format_type)
        except ImportError:
            pass

    if changes_content is None and report_data.get("stats"):
        try:
            from trendradar.report.changes import generate_changes_for_report
            changes_content = generate_changes_for_report(report_data, format_type)
        except ImportError:
            pass

    if cross_platform_content is None and report_data.get("stats"):
        try:
            from trendradar.report.aggregator import aggregate_cross_platform, get_top_cross_platform_titles, format_cross_platform_card
            aggregate_cross_platform(report_data["stats"])
            cross_titles = get_top_cross_platform_titles(report_data["stats"])
            cross_platform_content = format_cross_platform_card(cross_titles, format_type)
        except ImportError:
            pass
    ...
    # 注入前言卡片（TL;DR、变化对比、跨平台热点）
    preface_parts = []
    if tldr_content:
        preface_parts.append(tldr_content)
    if changes_content:
        preface_parts.append(changes_content)
    if cross_platform_content:
        preface_parts.append(cross_platform_content)

    if preface_parts:
        preface_text = ""
        for j, part in enumerate(preface_parts):
            if j > 0:
                sep = f"\n{feishu_separator}\n\n" if format_type == "feishu" else "\n---\n\n" if format_type == "dingtalk" else "\n\n"
                preface_text += sep
            preface_text += part
        ...
        test_content = current_batch + preface_text
        if len(test_content.encode("utf-8")) + len(base_footer.encode("utf-8")) < max_bytes:
            ...
            # 尝试把前言放在新批次
            preface_with_header = base_header + preface_text
            current_batch = _safe_new_batch(
                preface_with_header, base_footer, max_bytes, base_header, batches
            )
            current_batch_has_content = True
